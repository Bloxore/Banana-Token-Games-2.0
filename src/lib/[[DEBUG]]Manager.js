...
export class Manager extends Phaser.Scene {
  ...
  create() {
    ...
    this.sceneDisplay = this.add.group();
    // %% END DEBUG %%
  }

  // %% DEBUG %%
  /* Visual display of every available scene */
  generateSceneList() {
    let scenes = this.scene.manager.keys;
    delete scenes.Manager; // Being able to switch to this scene will only cause problems
    let sceneNames = Object.keys(scenes);
    let numScenes = sceneNames.length;

    // Height of each item is 50px
    let itemHeight = 20;
    let itemWidth = 200;
    ...
    let textLeftPadding = 2;

    let titleHeight = 30;

    let height = (itemHeight + itemPadding*2)*numScenes + 30;
    let width = itemWidth + widthPadding*2;
    // Flush against left side of the screen
    let x = 0;
    let y = this.game.config.height / 2 - height / 2;
    ...
    let title = this.add.text(x + widthPadding + textLeftPadding, y + titleHeight / 8, "States", {
      fontFamily: "Arial",
      fontStyle: "bold",
      fontSize: "22px",
      color: "#000000"
    })
    this.sceneDisplay.add(title);

    this.sceneDisplay.add(box);

    // Create text scene list
    for (let i in sceneNames) {
      let name = sceneNames[i];
      let nameBox = this.add.graphics();
      nameBox.fillStyle(0xCCCCCC);
      nameBox.fillRect(x + widthPadding, y + titleHeight + i*(itemHeight + itemPadding*2) + itemPadding, itemWidth, itemHeight);
      this.sceneDisplay.add(nameBox);

      let hitArea = this.add.zone(x + widthPadding, y + titleHeight + i*(itemHeight + itemPadding*2) + itemPadding, itemWidth, itemHeight);
      hitArea.setInteractive();
      hitArea.setOrigin(0,0);
      this.sceneDisplay.add(hitArea);
      hitArea.on("pointerdown", () => {

        // Then stop the current scene
        for (let scene of this.scene.manager.scenes) {
          if (scene.scene.key != "Manager" && scene.scene.isPaused()) {
            scene.scene.run(scene.scene.key);
      ...
      let nameText = this.add.text(x + widthPadding + textLeftPadding, y + titleHeight + i*(itemHeight + itemPadding*2) + itemPadding, name,{
        fontFamily: "Arial",
        fontStyle: "bold",
        fontSize: "11px",
        color: "#000000"
      });
      this.sceneDisplay.add(nameText);
    }
  }

  destroySceneList() {
    this.sceneDisplay.clear(true, true);
  }
  // %% END DEBUG %%
